...
source scripts/fedora/spawn.conf
...
if [ ! -f ${TEMPLATE} ]; then
	...
	echo -e "You can change the path and image for the desired template in scripts/fedora31/spawn.conf.\n"
	exit 1
else
...
--os-type=linux \
--os-variant=rhel7.5 \
--dry-run --print-xml > /tmp/fedora31.xml

virsh define --file /tmp/fedora31.xml && rm /tmp/fedora31.xml
